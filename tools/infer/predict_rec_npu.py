...
class TextRecognizer(BaseOCRV20):
    ...
    def __call__(self, img_list):
        # print(f'type(img_list)={type(img_list)}, len(img_list)={len(img_list)}')
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            # print(f'self.rec_algorithm == {self.rec_algorithm}')  # CRNN
            for ino in range(beg_img_no, end_img_no):
            ...
            if self.rec_algorithm == "SRN":
                ...
                with torch.no_grad():
                    ...
                    if self.use_npu:
                        inp = inp.npu()
                        encoder_word_pos_inp = encoder_word_pos_inp.npu()
                        gsrm_word_pos_inp = gsrm_word_pos_inp.npu()
                        gsrm_slf_attn_bias1_inp = gsrm_slf_attn_bias1_inp.npu()
                        gsrm_slf_attn_bias2_inp = gsrm_slf_attn_bias2_inp.npu()
